...
def sign_post(token):
    ...
    if not file: return abort(400, "signature required")

    orig = fs.find_one({"metadata.type":"quote_original","metadata.quote_id": q.get("quote_id","")})
    if not orig: return abort(400, "original pdf missing")
    ...
    quotes.update_one({"_id": q["_id"]}, {"$set": {
        ...
        "status": (
            "fully_signed"
            if (
                (role == "buyer" and q.get("seller", {}).get("signed"))
                or (role == "seller" and q.get("buyer", {}).get("signed"))
            )
            else f"{role}_signed"
        ),
    }})
